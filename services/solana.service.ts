...
export class SolanaBlockchainService {
    ...
    static disconnectWallet = async ({sharedSecret, session} : DisconnectWalletParams) : Promise<string> => {
    ...
    static claimMomentoToken = async ({
        ...
        try{
            ...
            const res = await fetch('http://192.168.50.136:3000/solana/claim-token', {
                ...
                body: JSON.stringify({ publicKey: phantomWalletPublicKey.toBase58(), amount: amount }),
            });

            const data = await res.json();
            console.log(data.instruction,"333")

            const transaction = Transaction.from(Buffer.from(data.instruction, 'base64'));
            console.log("3.1")
    ...
    static mintNFT = async({ image, phantomWalletPublicKey, session, setSubmitting, sharedSecret } : MintNFTParams) => {
        if (!phantomWalletPublicKey) throw new WalletNotConnectedError();
        ...
        const res = await fetch('http://192.168.50.136:3000/solana/mint-nft', {
            ...
            body: JSON.stringify({ publicKey: phantomWalletPublicKey.toBase58(), image: image }),
        });
